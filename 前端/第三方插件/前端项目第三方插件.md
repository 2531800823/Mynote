...
### hotkeys-js 快捷键添加，不依赖于框架

### 编辑器 tiptap 可以像飞书一样的编辑
...
##### 标签选择 选择 选择框 框选 框选工具 Viselect

##### react-json-view 可折叠JSON 对象，react

##### react 快捷键 react-hotkeys-hook 可以以字符串的形式来绑定快捷键
...
##### pkg-dir 查找项目 一层一层找 找到第一个  package.json 返回目录

##### locate-path 获取磁盘上多个路径的第一个路径
...
##### import-local 如果全局和本地都有一个包，优先选择本地的

##### glob  使用shell方式匹配 文件

##### node-ssh node ssh的方式链接服务器，操作

##### simple-git git 操作，代码操作git

##### terminal-link  创建一个在终端可以点击的链接。使用 ssh 链接服务器
